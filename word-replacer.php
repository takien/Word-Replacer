...
if (!class_exists('WordReplacer')) {
	...
	wp_register_script('word_replacer_editarea', plugin_dir_url( __FILE__ ) . 'editarea/edit_area_full.js');
	wp_enqueue_script('word_replacer_editarea');
	...
	//<![CDATA[
	  ...
	  function loadCodeEditor(htmlid) {
	  	var cfg = {
			id: htmlid
			...
			,syntax: "html"
			,plugins: "charmap"
			,toolbar: "fullscreen, |, search, go_to_line, |, undo, redo, |, select_font, charmap, |, syntax_selection, |, change_smooth_selection, highlight, reset_highlight, |, word_wrap, |, help"
			,syntax_selection_allow: "html,css,js,php"
			,is_multi_files: false
			,show_line_colors: true
			,word_wrap: false
		};
		editAreaLoader.init(cfg);
	  }
	...
	//]]>
		...
		<style type="text/css">
			.strip_backslash {
		  	...
		  	#word-replacer-list textarea {
				min-height: 5rem;
				max-height: 15rem;
				font-family: 'Courier New', Courier, monospace;
			}

			#word-replacer-list th {
				font-size: 0.6rem;
				padding: 0.3rem;
				text-align: center;
			}
	...
	$this->word_replacer_post();
		...
		<thead>
		  ...
		  <tr>
			<th width="25"><span title="delete entry">Del</span></th>
			<th width="20%"><span title="original text to be replaced">Original</span></th>
			<th width="5">&nbsp;</th>
			<th><span title="replacement text for left text">Replacement</span></th>
			<th class="replacer_expandable" width="20"><span title="applies to blog posts">Pos</span></th>
			<th class="replacer_expandable" width="20"><span title="applies to comments">Com</span></th>
			<th class="replacer_expandable" width="20"><span title="applies to static pages">Pag</span></th>
			<th class="replacer_expandable" width="20"><span title="applies to titles">Tit</span></th>
			<th class="replacer_expandable" width="20"><span title="applies to bbPress">bbP</span></th>
			<th class="replacer_expandable" width="20"><span title="ignore case">i/c</span></th>
			<th class="replacer_expandable" width="20"><span title="whole word matching">w/w</span></th>
			<th class="replacer_expandable" width="20"><span title="this is a RegEx (regular expression)">Reg</span></th>
		...
		<?php
			foreach($this->fields as $field=>$name) {
				if( 'original' == $field ) {
					...
					?>
					<td>
					  <textarea style="resize:vertical;width:100%" name="replacement[<?php echo $i;?>]" id="replacement_<?php echo $i;?>"><?php echo $this->esc_textarea($wrdb['replacement']); ?></textarea><label class="strip_backslash"><input type="checkbox" value="1" name="strip_backslash[<?php echo $i;?>]"/> Strip backslash?</label><br />
					  <span onclick="loadCodeEditor('replacement_<?php echo $i;?>');" style="display: inline-block; border: 1px black solid; background-color: #C0C0C0; color: #000000; padding: 5px; cursor: pointer;">Code Editor</span>
					</td>
					<?php
				}
				else {
